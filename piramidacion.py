...
class Piramidacion(CalculoIMSSObrero,ISR,SBC):
    ...
    def calculo_vales(self, gross_salary:float) -> int:
        return int(gross_salary * 0.05) if int(gross_salary * 0.05) <= 1500 else 1500
    ...
    def df_iteraciones(self, rango:int, incremento:float, approach_income:float, with_vouchers=0):
        ...
        net_salary = []
        if with_vouchers == 1:
            for sueldo, vale, isr, imss in zip(sueldos, vouchers, isrs, imss_obs):
                net_salary.append(sueldo - isr - imss + vale)
        else:
            for sueldo, vale, isr, imss in zip(sueldos, isrs, imss_obs):
                net_salary.append(sueldo - isr - imss)
    ...
    def df_ta(self, buscar_valor:float, with_vouchers=0):
        ...
        df = df[(df['Neto']>=(buscar_valor-0.01*2))&(df['Neto']<=(buscar_valor+0.01*3))]
